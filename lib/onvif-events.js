...
class OnvifEventClient {
    ...
    async start() {
        // Try both common Reolink event service paths
        const candidates = [
            `${this._baseUrl}/onvif/event_service`,
            `${this._baseUrl}/onvif/Events`,
            `${this._baseUrl}/onvif/events`,
        ];
    ...
    <tev:CreatePullPointSubscription>
      <tev:InitialTerminationTime>PT60S</tev:InitialTerminationTime>
        ...
        if (this.log) {
            this.log.debug(`ONVIF CreatePullPoint [${res.status}] from ${this.host} (len=${res.body.length}): ${res.body.substring(0, 1000)}`);
            if (res.body.length > 1000) this.log.debug(`ONVIF response cont. ${this.host}: ${res.body.substring(1000, 2000)}`);
            if (res.body.length > 2000) this.log.debug(`ONVIF response cont2. ${this.host}: ${res.body.substring(2000, 3000)}`);
        }
        ...
        if (subRefMatch) {
            this.log.debug(`ONVIF ${this.host}: SubscriptionReference block: ${subRefMatch[1].substring(0, 300)}`);
        ...
        // Log all Address tags found for diagnosis
        if (!subscriptionUrl) {
            const allAddrs = [...res.body.matchAll(/<[^:>]*:?Address[^>]*>\s*([^<]+?)\s*<\/[^:>]*:?Address>/gi)];
            this.log.warn(`ONVIF ${this.host}: no subscription URL found. All Address values: ${allAddrs.map(m => m[1]).join(' | ')}`);
