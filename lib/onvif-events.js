...
class OnvifEventClient {
    ...
    <tev:CreatePullPointSubscription>
      <tev:InitialTerminationTime>PT60S</tev:InitialTerminationTime>
        ...
        if (this.log) this.log.debug(`ONVIF CreatePullPoint response from ${this.host}: ${res.body.substring(0, 2000)}`);
        ...
        // Try SubscriptionReference block first
        ...
        // Fallback: any Address tag with http
        if (!subscriptionUrl) {
        ...
        // Fallback: some Reolink cams reuse event_service URL for PullMessages
        if (!subscriptionUrl) {
            this.log.warn(`ONVIF ${this.host}: no subscription URL in response, using event_service as fallback`);
