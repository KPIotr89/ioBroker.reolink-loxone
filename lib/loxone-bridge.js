...
class LoxoneBridge {
    ...
    /**
     * Send a custom named event to Loxone Virtual Input (e.g. whiteLed state)
     * @param {string} cameraName
     * @param {string} eventName - event suffix (e.g. 'whiteLed')
     * @param {number} value - 0 or 1
     */
    async sendCustomEvent(cameraName, eventName, value) {
        const safeName = cameraName.replace(/[^a-zA-Z0-9_-]/g, '_');
        const safeEvent = eventName.replace(/[^a-zA-Z0-9_-]/g, '_');
        await this.sendEvent(`Reolink_${safeName}_${safeEvent}`, value);
    }
