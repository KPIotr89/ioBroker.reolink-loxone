...
class ReolinkAPI {
    ...
    /**
     * Get doorbell config (supported on Reolink Video Doorbell cameras)
     */
    async getDoorbell(channel) {
        const ch = channel !== undefined ? channel : this.channel;
        return this._cmd('GetDoorbell', { channel: ch });
    }

    /**
     * Get current push/webhook configuration from camera
     */
    async getPush(channel) {
        const ch = channel !== undefined ? channel : this.channel;
        return this._cmd('GetPushV20', { channel: ch });
    }

    /**
     * Configure camera to send HTTP push alerts to a webhook URL.
     * @param {object} pushConfig - { channel, enable, url, scheduleEnable }
     */
    async setPush(pushConfig) {
        return this._cmd('SetPushV20', { Push: pushConfig }, 0);
    }
    ...
    /**
     * Set white light (spotlight) - uses direct auth for maximum compatibility
     * @param {object} ledConfig
     * @param {number} ledConfig.channel - Channel number
     * @param {number} ledConfig.state - 0=off, 1=on
     * @param {number} [ledConfig.mode=0] - 0=auto, 1=manual, 3=schedule
     * @param {number} [ledConfig.bright=100] - Brightness 0-100
     */
    async setWhiteLed(ledConfig) {
        const payload = {
            WhiteLed: {
                ...
                LightingSchedule: ledConfig.LightingSchedule || {
                    EndHour: 0,
                    EndMin: 0,
                    StartHour: 0,
                    StartMin: 0,
                },
                wlAiDetectType: ledConfig.wlAiDetectType || {
                    dog_cat: 0,
                    face: 0,
                    people: 0,
                    vehicle: 0,
        ...
        // channel in URL + no action field — exactly matching the Loxone working format
    ...
    /** Get legacy push notification config (older firmware) */
    async getPushLegacy() {
        return this._cmd('GetPush');
    }

    /** Set legacy push notification config (older firmware) */
    async setPushLegacy(pushConfig) {
