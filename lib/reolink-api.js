...
class ReolinkAPI {
     ...
     * @returns {Promise<object>} response value
     */
    async _cmdDirect(cmd, param = {}, action = 0) {
        const url = `/api.cgi?cmd=${cmd}&user=${encodeURIComponent(this.username)}&password=${encodeURIComponent(this.password)}`;
        const body = [{ cmd, action, param }];
    ...
    async getWhiteLed(channel) {
        const ch = channel !== undefined ? channel : this.channel;
        return this._cmdDirect('GetWhiteLed', { channel: ch });
    }
    ...
    async setWhiteLed(ledConfig) {
        ...
        return this._cmdDirect('SetWhiteLed', payload, 0);
    }
