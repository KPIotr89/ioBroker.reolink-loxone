...
class ReolinkAPI {
     ...
     * @returns {Promise<object>} response value
     */
    async _cmdDirect(cmd, param = {}, action = 0, urlParams = {}) {
    ...
    getRtspUrl(channel, stream = 'main') {
        const ch = channel !== undefined ? channel : this.channel;
        const streamPath = stream === 'sub' ? 'h264Preview_01_sub' : (stream === 'ext' ? 'h264Preview_01_ext' : 'h264Preview_01_main');
    ...
    /** Get white light (spotlight) settings */
    async getWhiteLed(channel) {
        const ch = channel !== undefined ? channel : this.channel;
        return this._cmdDirect('GetWhiteLed', { channel: ch }, 0, { channel: ch });
    }
    ...
    async setWhiteLed(ledConfig) {
        ...
        // channel must also be in URL query string (as Loxone does it)
        return this._cmdDirect('SetWhiteLed', payload, 0, { channel: ledConfig.channel || 0 });
    }
