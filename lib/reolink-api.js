...
class ReolinkAPI {
     ...
     * Returns doorbell settings including visitor detection config
     */
    ...
    /**
     * Get visitor alarm state — checks if doorbell button was pressed.
     * Uses GetAiState: visitor.alarm_state = 1 when button pressed.
     * Also usable on cameras that support visitor AI detection (not just doorbell cameras).
     */
    async getVisitorState(channel) {
        const ch = channel !== undefined ? channel : this.channel;
        // Try doorbell-specific alarm first, fall back to AI state
        try {
            return this._cmd('GetAiState', { channel: ch });
        } catch (_) {
            return null;
